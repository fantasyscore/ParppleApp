...
import { AppText, BLACK, INTER_SEMI_BOLD, LIGHT_BLACK, OPECITY_DARK, TWELVE } from "./AppText";
import { keywordRightArrow } from "../helper/ImageAssets";
import { TouchableOpacityView } from "./TouchableOpacityView";

const EditButtonCommon = ({ Icons, title, first, filluptext, onPress, setting, setToggleShow, togleShow, toggle, setSelectFtCm, selectFtCm, Distance, inText, inTextTwo }: any) => {
    return (
        <TouchableOpacityView onPress={onPress} style={[styles.container, { marginTop: first ? metrics.hp2 : metrics.hp0_5, borderWidth: setting ? 0 : filluptext ? metrics.hp0 : metrics.hp0_1, borderColor: colors.red }]}>
            ...
            <View style={{ flexDirection: "row", alignItems: "center" }}>
                {filluptext &&
                    <AppText style={{textTransform:"capitalize"}} color={LIGHT_BLACK} weight={INTER_SEMI_BOLD} type={TWELVE}>
                        {filluptext}{"   "}
                    </AppText>
                }
                {Distance &&
