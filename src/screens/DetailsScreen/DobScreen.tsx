...
import { AppText, fontSize, INTER_MEDIUM, INTER_SEMI_BOLD, LIGHT_BLACK, OPECITY, SCHEHERAZADE_BOLD, SIXTEEN, TWELVE, WHITE } from "../../common/AppText";
...
const DobScreen = () => {
    ...
    const formatDate = (date: any) => {
        ...
        return date
            ...
            .replace(" ", " ")  // spacing fix
            .replace(/(\d{2}\s\w{3})\s(\d{4})/, "$1, $2"); // add comma
    };
    const formattedDate = date?.toISOString().split("T")[0];
    const onSubmit = () => {
        // if (!formattedDate) return toastAlert.showToastError("Please add your DOB");
        const data = {
            ...addProfileData,
    ...
    return (
        <AppSafeAreaView>
            ...
            <View style={styles.container}>
                ...
                <View style={{ paddingHorizontal: metrics.hp2 }}>
                    ...
                    <View style={styles.dobContainer}>
                            ...
                            {dateDigits.map((digit, index) => (
                                <View
                                    ...
                                    style={[
                                        ...
                                        {
                                            marginLeft:
                                                index === 0 ? 0 : index === 2 || index === 4 ? metrics.hp1_5 : metrics.hp1,
                                        },
                                    ]}>
                                    <AppText
                        ...
                        <TouchableOpacityView onPress={() => setOpen(true)}>
                            <FastImage
                                source={calendarIcon}
                                resizeMode="contain"
            ...
            <GoButton colortrue={date} onPress={() => !formattedDate ? toastAlert.showToastError("Please add your DOB"): setModalVisible(true)} />
            <DatePicker
                mode="date"
                modal
                open={open}
                date={date || minAgeDate}
            ...
            <Modal
                ...
                <View style={styles.centeredView}>
                    <View style={styles.confirmContainer}>
                        ...
                        <AppText weight={SCHEHERAZADE_BOLD} style={{ fontSize: fontSize(30), textAlign: "center" }}>
                            {date ? `You’re ${calculateAge(date)}` : ""}
                        </AppText>
                        <AppText style={{ textAlign: "center", marginTop: -metrics.hp2 }} type={TWELVE} color={LIGHT_BLACK}>
                            Born {formatDate(date)}
                        </AppText>
                        <AppText style={{ textAlign: "center" }} type={TWELVE} color={LIGHT_BLACK}>
                            Can’t change later. Confirm it now.
                        </AppText>
                        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: metrics.hp2, marginTop: metrics.hp3 }}>
                            <TouchableOpacityView onPress={() => setModalVisible(false)} style={styles.ediButton}>
                                <AppText color={LIGHT_BLACK} weight={INTER_SEMI_BOLD} type={TWELVE}>
                                    Edit
                                </AppText>
                            </TouchableOpacityView>
                            <TouchableOpacityView onPress={() => onSubmit()} style={[styles.ediButton, { backgroundColor: colors.purple }]}>
